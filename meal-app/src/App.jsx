            ...
            {isManager ? (
              <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 space-y-4">
                ...
                <form onSubmit={updateBills} className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 block mb-1">WiFi (Last Date: {DUE_DATES.WIFI})</label>
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 block mb-1">বিদ্যুৎ (Last Date: {DUE_DATES.CURRENT})</label>
                  ...
                  <div>
                    <label className="text-[10px] font-bold text-slate-400 block mb-1">ভাড়া (Last Date: {DUE_DATES.RENT})</label>
              ...
              <div className="bg-indigo-50 p-5 rounded-3xl text-indigo-900 border border-indigo-100 grid grid-cols-3 gap-4 text-center">
                 <div><span className="text-[10px] uppercase font-bold opacity-60">WiFi (Last Date: {DUE_DATES.WIFI})</span><p className="font-black text-lg">৳{sharePerHead.wifi.toFixed(0)}</p></div>
                 <div className="border-x border-indigo-200"><span className="text-[10px] uppercase font-bold opacity-60">বিদ্যুৎ (Last Date: {DUE_DATES.CURRENT})</span><p className="font-black text-lg">৳{sharePerHead.current.toFixed(0)}</p></div>
                 <div><span className="text-[10px] uppercase font-bold opacity-60">ভাড়া (Last Date: {DUE_DATES.RENT})</span><p className="font-black text-lg">৳{sharePerHead.rent.toFixed(0)}</p></div>
              </div>
            )}
            ...
            {isManager ? (
              <div className="space-y-6">
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 space-y-4">
                    <h3 className="font-bold text-slate-800 flex items-center gap-2"><Zap size={18} className="text-amber-500"/> ফিক্সড বিল</h3>
                    <form onSubmit={updateBills} className="grid grid-cols-3 gap-2">
                      <div><label className="text-[10px] font-bold text-slate-400 block mb-1">WiFi (Last: {DUE_DATES.WIFI})</label><input name="wifi" type="number" defaultValue={safeNum(bills.wifi)} className="w-full p-2 bg-slate-50 rounded-lg text-xs font-bold outline-none" /></div>
                      <div><label className="text-[10px] font-bold text-slate-400 block mb-1">বিদ্যুৎ (Last: {DUE_DATES.CURRENT})</label><input name="current" type="number" defaultValue={safeNum(bills.current)} className="w-full p-2 bg-slate-50 rounded-lg text-xs font-bold outline-none" /></div>
                      <div><label className="text-[10px] font-bold text-slate-400 block mb-1">ভাড়া (Last: {DUE_DATES.RENT})</label><input name="rent" type="number" defaultValue={safeNum(bills.rent)} className="w-full p-2 bg-slate-50 rounded-lg text-xs font-bold outline-none" /></div>
                      <button className="col-span-3 bg-slate-800 text-white p-2 rounded-xl text-xs font-bold mt-2">বিল সেভ করুন</button>
                    </form>
                  </div>
                </div>
                ...
                <div className="bg-gradient-to-r from-emerald-500 to-teal-600 p-6 rounded-3xl shadow-lg text-white space-y-4">
                  ...
                  <form onSubmit={addDeposit} className="flex gap-2">
                    <select name="memberId" className="flex-1 p-3 rounded-xl bg-white/20 border border-white/30 text-white outline-none text-sm font-bold placeholder-white">
                    ...
                    <button className="bg-white text-emerald-600 px-6 rounded-xl font-black text-sm shadow-lg">জমা</button>
                  </form>
                </div>
              ...
              {members.map(m => {
                ...
                <div key={m.id} className="bg-white p-3 rounded-2xl shadow-sm border border-slate-100 flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="font-bold text-sm text-slate-700 flex items-center gap-1">{name} {m.isManagerTag && <Crown size={12} className="text-amber-500" fill="currentColor"/>}</span>
